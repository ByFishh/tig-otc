...
import { Box, Text, Card as RadixCard, Flex, Badge, Grid } from "@radix-ui/themes";
import React, { memo } from "react";
import { v4 as uuidv4 } from "uuid";
import { useCard } from "./Card.logic";

const propsAreEqual = (prevProps: Readonly<ICard>): boolean => !prevProps.forceRender;
              ...
              {props.data.map((d) => (
                <Flex key={uuidv4()} style={{ flexFlow: "column" }}>
                  <Text as="p" size="2" mb="0" color="gray">
                    {d.title} ({d.unit}){" "}
                    {d.percentage && (
                      <Badge color={d.percentage > 0 ? "jade" : "ruby"} radius="full">
                        {d.percentage > 0 && "+"}
                        {d.percentage.toFixed(1)}%
                      </Badge>
                    )}
                  </Text>
                  {logic.getValue(Number(d.value), d.unit)}
                </Flex>
              ))}
            </>
          )}
